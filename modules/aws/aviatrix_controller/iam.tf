...
resource "aws_iam_role" "aviatrix_role_ec2" {
  name               = length(var.tag_prefix) > 0 ? "${var.tag_prefix}-role-ec2" : var.aws_iam_role_ec2_name
...
resource "aws_iam_role" "aviatrix_role_app" {
  name               = length(var.tag_prefix) > 0 ? "${var.tag_prefix}-role-app" : var.aws_iam_role_app_name
...
resource "aws_iam_policy" "aviatrix_assume_role_policy" {
  name        = length(var.tag_prefix) > 0 ? "${var.tag_prefix}-assume-role-policy" : var.aws_iam_assume_policy_role_policy_name
  path        = "/"
...
resource "aws_iam_policy" "aviatrix_app_policy" {
  name        = length(var.tag_prefix) > 0 ? "${var.tag_prefix}-app-policy" : var.aws_iam_app_policy_name
  path        = "/"
